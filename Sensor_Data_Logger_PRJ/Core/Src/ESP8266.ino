...
#ifdef MODE_AP
// For AP mode:
const char *ssid = "Username";  // You will connect your phone to this Access Point
const char *pw = "Password"; // and this is the password
IPAddress ip(192, 168, 0, 1); // From Server and Android , connect to this IP
...
#ifdef MODE_STA
// For STATION mode:
const char *ssid = "Username";  // Your ROUTER SSID
...
void setup() {
  ...
  // from Server and Android you must connect to the IP of the ESP
  WiFi.mode(WIFI_STA);
  WiFi.begin(ssid, pw);
...
void loop() {
  ...
  if(client.available()) {
    while(client.available()) {
      buf1[i1] = (uint8_t)client.read(); // read char from client (Server and Android)
